...
function MediaTile({ item, mediaType, size, selected, selectMode, onPress, onLongPress }: {
  ...
  const thumb = mediaType === 'photos'
    ? ((item as MomentPhoto).thumbnail_url || (item as MomentPhoto).r2_url)
    : ((item as MomentVideo).thumbnail_url || `https://img.youtube.com/vi/${(item as MomentVideo).youtube_id}/mqdefault.jpg`)
  const isActive = (item as MomentVideo).is_active !== false
...
export default function MomentsAdminScreen() {
  ...
  const TILE_SIZE = Math.floor((width - 24 - 9) / 4)
  ...
  const [reorderItems, setReorderItems] = useState<(MomentPhoto | MomentVideo)[]>([])
  const [reorderAlbumsList, setReorderAlbumsList] = useState<TmAlbum[]>([])
  ...
  const reorderItemsRef = useRef<(MomentPhoto | MomentVideo)[]>([])
  const reorderAlbumsRef = useRef<TmAlbum[]>([])
  const rdDraggingIdxRef = useRef(-1)
  const rdAnchorIdxRef = useRef(-1)

  const SLOT_H_ITEM = 66
  const SLOT_H_ALBUM = 58
  ...
  function handleTilePress(item: MomentPhoto | MomentVideo) {
    if (selectMode) {
      setSelectedIds(prev => {
  ...
  function handleTileLongPress(item: MomentPhoto | MomentVideo) {
    setSelectMode(true)
  ...
  async function handleBulkToggle() {
    ...
    const allActive = ids.every(id => {
      ...
      return item ? (item as MomentVideo).is_active !== false : false
    })
    const newActive = !allActive
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) { Alert.alert('Error', 'Not signed in.', [{ text: 'OK' }]); return }
    const apiPath = mediaTab === 'photos' ? 'timeless-moments' : 'timeless-moments/videos'
    const res = await fetch(
      `https://www.bazidpur.com/api/${apiPath}?_t=${encodeURIComponent(session.access_token)}`,
      ...
      if (editingId) {
        ...
        for (let i = 0; i < form.images.length; i++) {
          ...
          const newPhoto = await webApiUpload(
            'https://www.bazidpur.com/api/timeless-moments', uri,
          ...
          if (selectedPhotoAlbumId && (newPhoto as any).id) { // eslint-disable-line @typescript-eslint/no-explicit-any
            await supabase.from('timeless_moments').update({ album_id: selectedPhotoAlbumId }).eq('id', (newPhoto as any).id) // eslint-disable-line @typescript-eslint/no-explicit-any
          }
  ...
  function showActions(item: MomentPhoto | MomentVideo) {
    const active = (item as MomentVideo).is_active !== false
  ...
  async function savePhotoReorder() {
    ...
    try {
      ...
      if (mediaTab === 'photos') setPhotos(updated as MomentPhoto[])
  ...
  async function saveAlbumReorder() {
    ...
    try {
      ...
      const res = await fetch(
        `https://www.bazidpur.com/api/tm-albums?_t=${encodeURIComponent(session.access_token)}`,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f2f2f7' }}>
      ...
      {/* Photo/video reorder — full-screen overlay, plain View (no Modal) */}
      {photoReorderMode && (
            ...
            <View style={{ paddingTop: LIST_TOP_REORDER }}>
              {reorderItems.map(it => {
                ...
                const thumb = mediaTab === 'photos'
                  ? ((it as MomentPhoto).thumbnail_url || (it as MomentPhoto).r2_url)
                  : `https://img.youtube.com/vi/${(it as MomentVideo).youtube_id}/mqdefault.jpg`
                return (
